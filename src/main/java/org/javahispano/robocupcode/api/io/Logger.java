 ...
 * https://robocode.sourceforge.io/license/epl-v10.html
 */
package org.javahispano.robocupcode.api.io;

import java.io.ByteArrayOutputStream;
import java.io.PrintStream;

import org.javahispano.robocupcode.api.util.StringUtil;
...
public class Logger {
	...
	private static IMatchListener logListener;
	
	private final static StringBuffer logBuffer = new StringBuffer();

	public static void setLogListener(IMatchListener logListener) {
		...
		if (logListener == null) {
			...
			synchronized (logBuffer) {
				if (!HiddenAccess.isSafeThread()) {
					...
					logListener.onMatchMessage(new MatchMessageEvent(logBuffer + s));
	...
	public static void logError(String s) {
		if (logListener == null) {
			...
			logListener.onBattleError(new MatchErrorEvent(s));
		}
	}
